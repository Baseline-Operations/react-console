...
import { Cell, BoundingBox, createEmptyCell, isCellTransparent } from './types';
...
export class CompositeBuffer {
  ...
  private compositeCellInto(dest: Cell, source: Cell, _opacity: number): void {
    // Check z-index - higher z-index always wins
    const sourceZ = source.zIndex || 0;
    const destZ = dest.zIndex || 0;

    // If source has higher z-index, it takes precedence completely
    if (sourceZ > destZ) {
    ...
    // If same z-index, use normal compositing (later content overwrites)
    if (sourceZ === destZ) {
      if (!isCellTransparent(source)) {
        dest.char = source.char;
        dest.foreground = source.foreground;
    ...
    // If source has lower z-index, don't overwrite (dest has higher priority)
  }
