...
export class BufferRenderer {
  ...
  render(root: Node, options: Partial<BufferRenderOptions> = {}): void {
    ...
    // 6. Flush to terminal - static mode uses simple output
    if (fullOptions.mode === 'static') {
      this.flushStatic(root);
    } else if (fullOptions.fullRedraw || this.isFirstRender) {
      this.displayBuffer.flush(process.stdout, fullOptions.clearScreen);
    } else {
    ...
    for (let y = bounds.height - 1; y >= 0; y--) {
      ...
      for (let x = 0; x < bounds.width; x++) {
        ...
        if (cell) {
          ...
          if (cell.background && cell.background !== 'transparent' && cell.background !== 'inherit') {
            hasContent = true;
            break;
          }
        }
  ...
  private checkAndHandleResize(): void {
    ...
    if (dims.columns !== this.compositeBuffer.width || 
        dims.rows !== this.compositeBuffer.height) {
  ...
  private createLayers(node: Node): void {
    ...
    if (!node.parent) {
      this.compositeBuffer.createLayer(node.id, 0, {
        x: 0,
        y: 0,
        width: dims.columns,
        height: dims.rows,
  ...
  private getNodeBounds(node: Node): BoundingBox {
    ...
    if ('computeLayout' in node) {
      const dims = getTerminalDimensions();
      const layout = (node as any).computeLayout({
  ...
  private renderNodeTree(node: Node): void {
    ...
    for (const child of node.children) {
      ...
      if (child.type === 'text' && node.type === 'text' && 
          child.content === node.content && !child.bounds) {
        continue;
      }
  ...
  private renderNode(node: Node): void {
    ...
    if ('computeStyle' in node) {
      const style = (node as any).computeStyle();
    ...
    if ('renderToCellBuffer' in node) {
      ...
      (node as any).renderToCellBuffer(context);
    } else {
    ...
    if (isInteractive) {
      componentBoundsRegistry.register(createComponentBounds(
        node as any, // ConsoleNode interface compatibility
        bounds.x,
        bounds.y,
        bounds.width,
        bounds.height
  ...
  private defaultNodeRender(node: Node, context: CellRenderContext): void {
    ...
    if (node.border && (node.border.show.top || node.border.show.right || 
  ...
  private renderTextContent(content: string, context: CellRenderContext): void {
    const { buffer, x, y, maxWidth, maxHeight, foreground, background, layerId, nodeId, zIndex } = context;
