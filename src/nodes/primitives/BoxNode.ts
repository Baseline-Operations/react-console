...
import type { LayoutConstraints, LayoutResult, ChildLayout, Dimensions } from '../base/mixins/Layoutable';
import type { StyleMap, DisplayMode } from '../base/types';
 ...
 * Composed with Stylable, Renderable, and Layoutable mixins
 */
export class BoxNode extends Stylable(Renderable(Layoutable(Node as any))) {
  ...
  computeLayout(constraints: LayoutConstraints): LayoutResult {
    ...
    const totalHorizontalSpace = borderWidth.left + borderWidth.right + this.padding.left + this.padding.right;
    const totalVerticalSpace = borderWidth.top + borderWidth.bottom + this.padding.top + this.padding.bottom;
    ...
    const contentMaxWidth = Math.max(1, Math.min(effectiveMaxWidth, maxTerminalWidth) - totalHorizontalSpace);
    const contentMaxHeight = effectiveMaxHeight ? Math.max(1, Math.min(effectiveMaxHeight, maxTerminalHeight) - totalVerticalSpace) : undefined;
    ...
    if (this.width !== null) {
      ...
      childAvailableWidth = Math.max(1, Math.min(effectiveAvailableWidth, maxTerminalWidth) - totalHorizontalSpace);
    }
    ...
    const contentConstraints: LayoutConstraints = {
      ...
      availableHeight: effectiveAvailableHeight !== undefined
    ...
    if (this.display === DisplayModeEnum.FLEX) {
      childLayouts = layoutEngine.layoutFlexbox(layoutNode as any, contentConstraints);
    } else if (this.display === DisplayModeEnum.GRID) {
      childLayouts = layoutEngine.layoutGrid(layoutNode as any, contentConstraints);
    } else {
      childLayouts = layoutEngine.layoutBlock(layoutNode as any, contentConstraints);
    }
    ...
    if (normalFlowChildren.length > 0) {
      ...
      let currentY = childLayouts.length > 0 
    ...
    const dimensions = this.calculateDimensions(childLayouts, borderWidth, layoutContext as Partial<RenderContext>);
  ...
  render(buffer: OutputBuffer, context: RenderContext): RenderResult {
    ...
    (this as any)._renderContext = context;
    ...
    if (this.children.length > 0) {
      ...
      for (const child of this.children) {
        ...
        if ('render' in child) {
          ...
          const safeMaxHeight = childContext.constraints.maxHeight ? Math.max(1, childContext.constraints.maxHeight) : undefined;
          const safeAvailableWidth = Math.max(1, childContext.constraints.availableWidth ?? Infinity);
          const safeAvailableHeight = childContext.constraints.availableHeight ? Math.max(1, childContext.constraints.availableHeight) : undefined;
          
          const childLayout = 'computeLayout' in child 
            ? (child as any).computeLayout({
          ...
          if (childLayout && childLayout.dimensions) {
            ...
            if (childWidth > 0 && childHeight > 0) {
              ...
              // Render the child
              const result = (child as any).render(buffer, fallbackChildContext);
              maxEndY = Math.max(maxEndY, result.endY);
  ...
  private calculateDimensions(
    ...
    if (this.display === DisplayModeEnum.FLEX || this.display === DisplayModeEnum.GRID) {
      ...
      if (flowChildLayouts.length > 0) {
        ...
        const rowGap: number = style.getProperty('rowGap') ?? (typeof gap === 'object' ? (gap.column ?? 0) : typeof gap === 'number' ? gap : 0);
        ...
        if (isRow || this.display === DisplayModeEnum.GRID) {
          ...
          const maxContentWidth = availableWidth - borderWidth.left - borderWidth.right - this.padding.left - this.padding.right;
          ...
          if (flowChildLayouts.length > 0) {
            contentAreaHeight = Math.max(...flowChildLayouts.map(l => l.bounds.y + l.bounds.height), 0);
          } else {
            contentAreaHeight = 0;
          }
        } else {
          ...
          const availableAfterMargin = maxContentWidth - (selfMargin.left || 0) - (selfMargin.right || 0);
          ...
          if (flowChildLayouts.length > 0) {
            ...
            const firstChildMargin = firstChild.node.margin || { left: 0, right: 0, top: 0, bottom: 0 };
      ...
      const totalWidth = borderWidth.left + this.padding.left + contentAreaWidth + this.padding.right + borderWidth.right;
      const totalHeight = borderWidth.top + this.padding.top + contentAreaHeight + this.padding.bottom + borderWidth.bottom;
      ...
      if (flowChildLayouts.length > 0) {
        ...
        maxWidth = Math.max(...flowChildLayouts.map(layout => {
        ...
        totalHeight = Math.max(1, lastLayout.bounds.y + lastLayout.bounds.height + (lastChildMargin.bottom || 0));
      } else {
      ...
      if (this.width !== null) {
        ...
        const availableContentWidth = context.constraints.availableWidth - borderWidth.left - borderWidth.right - this.padding.left - this.padding.right - (selfMargin.left || 0) - (selfMargin.right || 0);
      ...
      const minHeight = borderWidth.top + borderWidth.bottom + this.padding.top + this.padding.bottom + 1;
      const calculatedHeight = totalHeight + borderWidth.top + borderWidth.bottom + this.padding.top + this.padding.bottom;
      
      return {
        width: contentWidth + borderWidth.left + borderWidth.right + this.padding.left + this.padding.right,
