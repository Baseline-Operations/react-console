...
export function useSelection(componentRef: ConsoleNode | null | undefined): {
  ...
  const [selected, setSelectedState] = useState<string | number | (string | number)[] | null>(
    () => {
      ...
      if (
        componentRef.type === 'radio' ||
        componentRef.type === 'checkbox' ||
        componentRef.type === 'dropdown' ||
        componentRef.type === 'list'
      ) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (componentRef.type === 'dropdown' || componentRef.type === 'list') {
        ...
        const currentIndex =
          typeof componentRef.focusedIndex === 'number' ? componentRef.focusedIndex : -1;
