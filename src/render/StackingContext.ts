...
export class StackingContext {
  ...
  getRenderingOrder(): Node[] {
    ...
    const zeroContexts = this.childContexts.filter((c) => c.zIndex === 0);
    for (const context of zeroContexts) {
      order.push(...context.getRenderingOrder());
    }

    // 5. Positioned children with z-index 0
    const positionedZero = this.children.filter((c) => this.isPositioned(c.node) && c.zIndex === 0);
  ...
  static createsStackingContext(node: Node, style: ComputedStyle): boolean {
    ...
    if (
      (display === DisplayModeEnum.FLEX || display === DisplayModeEnum.GRID) &&
      zIndex !== null &&
      zIndex !== 0
    ) {
      return true;
