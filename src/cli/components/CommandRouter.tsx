...
import { parseCommandLineArgs } from '../utils/parser';
import { matchComponent, extractComponentMetadata } from '../utils/matcher';
import { generateHelpComponent } from '../help';
import { findHelp } from '../utils/helpMatcher';
import { isHelpAvailable } from '../utils/optionCollector';
import { findSimilarCommands } from '../utils/commandSuggestions';
import { addToHistory } from '../utils/history';
import { setDefaults, loadFromEnv, setCLIConfig } from '../utils/config';
import { resolveOptionAliases } from '../utils/optionResolver';
import { normalizeOptionValues } from '../utils/optionNormalizer';
import { collectCommandOptions } from '../utils/optionCollector';
import { shouldExitAfterCommand, getCommandExitCode } from '../utils/commandExecutor';
import { isVersionRequested, getAppVersion } from '../utils/version';
import { Node } from '../../nodes/base/Node';
const exit = Node.exit.bind(Node);
import { HelpWrapper } from '../utils/components/HelpWrapper';
import { VersionDisplay } from '../utils/components/VersionDisplay';
import { ValidationError } from '../utils/components/ValidationError';
import { UnknownCommandError } from '../utils/components/UnknownCommandError';
import { MiddlewareStopped } from '../utils/components/MiddlewareStopped';
import { RouteBlocked } from '../utils/components/RouteBlocked';
...
export interface CommandRouterProps extends StyleProps {
  ...
  defaultConfig?: Record<string, import('../utils/config').ConfigValue>;
...
export function CommandRouter({
  ...
  useEffect(() => {
    if (normalizedArgs.options && Object.keys(normalizedArgs.options).length > 0) {
      setCLIConfig(normalizedArgs.options as Record<string, import('../utils/config').ConfigValue>);
    }
