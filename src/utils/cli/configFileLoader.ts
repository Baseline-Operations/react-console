...
export async function loadConfigFromJSON(filePath: string): Promise<Config> {
  if (!isNodeJS()) {
    throw new Error('loadConfigFromJSON is only available in Node.js environment. Load the file yourself and use loadFromFile() instead.');
  }
  ...
  try {
    ...
    throw new Error(`Failed to load JSON config from ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function loadConfigFromYAML(filePath: string): Promise<Config> {
  if (!isNodeJS()) {
    throw new Error('loadConfigFromYAML is only available in Node.js environment. Load the file yourself and use loadFromFile() instead.');
  }
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('Cannot find module')) {
      throw new Error(`Failed to load YAML config: 'yaml' package is required. Install it with: npm install yaml`);
    }
    throw new Error(`Failed to load YAML config from ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function loadConfigFromTS(filePath: string): Promise<Config> {
  if (!isNodeJS()) {
    throw new Error('loadConfigFromTS is only available in Node.js environment. Load the file yourself and use loadFromFile() instead.');
  }
  ...
  try {
    ...
    throw new Error(`Failed to load config from ${filePath}. Make sure the file is compiled or use ts-node/tsx: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function loadConfigFile(
  ...
  if (!isNodeJS()) {
    throw new Error('loadConfigFile is only available in Node.js environment. Load the file yourself and use loadFromFile() instead.');
  }
  ...
  throw new Error(`No configuration file found at ${filePath} with extensions: ${extensions.join(', ')}`);
}
