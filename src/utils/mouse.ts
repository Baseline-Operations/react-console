 ...
 * - Button 64/65 = scroll wheel up/down
 *
...
export function parseMouseEvent(sequence: string): MouseEvent | null {
  ...
  if (sgrMatch) {
    ...
    let eventType: 'press' | 'release' | 'wheel' = sgrMatch[4] === 'M' ? 'press' : 'release';

    // Button encoding:
    // 0 = left, 1 = middle, 2 = right
    // 32 = motion while button pressed (drag)
    // 64 = scroll wheel up
    // 65 = scroll wheel down
    // Modifiers: 0x04 = shift, 0x08 = meta, 0x10 = ctrl

    const hasShift = (button & 0x04) !== 0;
    const hasCtrl = (button & 0x10) !== 0;
    const hasMeta = (button & 0x08) !== 0;

    // Check for scroll wheel events (button >= 64)
    const isScrollUp = (button & 0x40) !== 0 && (button & 0x01) === 0;
    const isScrollDown = (button & 0x40) !== 0 && (button & 0x01) !== 0;

    if (isScrollUp || isScrollDown) {
      return {
        x,
        y,
        button: isScrollUp ? 4 : 5, // 4 = scroll up, 5 = scroll down
        ctrl: hasCtrl,
        shift: hasShift,
        meta: hasMeta,
        eventType: 'wheel',
