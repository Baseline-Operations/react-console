...
[React Console API](../globals.md) / Spinner

# ~~Variable: Spinner()~~

> `const` **Spinner**: (`props`) => `ReactElement`\<`unknown`, `string` \| `JSXElementConstructor`\<`any`\>\> \| `null` = `ActivityIndicator`

Defined in: src/components/ui/ActivityIndicator.tsx:124
