...
function App() {
  ...
  return (
    <View>
      <StaticContent />
      <TextInput value={dynamicValue} onChangeText={setDynamicValue} />
    </View>
  );
}
```
...
function Form() {
  ...
  return (
    <View>
      <TextInput value={formData.name} onChangeText={(text) => updateForm('name', text)} />
      <TextInput value={formData.email} onChangeText={(text) => updateForm('email', text)} />
    </View>
  );
}
```
...
<TextInput
  ...
  onChangeText={setImportantField}
/>
...
function VirtualList({ items }: { items: Item[] }) {
  ...
  const visibleEnd = Math.min(visibleStart + visibleHeight, items.length);
  ...
  return (
    ...
    >
      {visibleItems.map((item, index) => (
        <Text key={item.id}>{items[visibleStart + index]?.label}</Text>
      ))}
    </Scrollable>
  );
}
